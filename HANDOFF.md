...
**2026-09-10 章序更新：**非監督式學習已移至支持向量機之後、深度學習補充之前；順序以 `tools/pages.py` 為準，既有 n 僅作固定元件識別碼。

**2026-09-06 定位紀錄：**目前共 26 頁。首頁順序為課前準備 → 正課 → 統計附錄 → Python 附錄；
...
章節順序依**講義與使用者明確指示**：非監督式（講義12）接在支持向量機（講義09）之後，
...
| PART 08 `bayes` | 講義 p.32、34 | Bayes 分類器在 0–1 損失下最佳：條件風險 `1 − p_c(x)` → 逐點取 argmax → 全期望公式推到全域；附損失／最佳解對照表與「Bayes 決策理論 ≠ 貝氏統計」的說明 |
