...
# ── P00 prologue ──────────────────────────────────────────────────────
BODIES["prologue"] = f"""
  <p>前面十幾週，每一章都有一個 <span class="orange">y</span>。有了 y，就能用預測誤差評估模型：
  ...
  <p>麻煩的地方是：<strong>沒有現成的真實群標籤，就不能直接用分類錯誤率判斷分群好壞。</strong>
  你得先說清楚想找什麼結構、用什麼證據評估。所以非監督式學習比監督式主觀得多，
  它的定位通常是<strong>探索式資料分析</strong>（exploratory data analysis）——
  產出的假設仍需進一步檢驗。</p>

  <p>這不表示不能留出資料做驗證。例如矩陣補全可以暫時遮住部分已觀測的格子，
  再用補值與真值的差距評估預測能力。<strong>能評估某個目標，不等於證明群具有實質意義。</strong></p>
...
{info("探索結果需要哪些證據", '''解讀非監督式學習的結果時，要留意<strong>過度解讀</strong>。
  任何時候把資料丟去分群，它<em>一定</em>會給你群——問題是那些群是真的子群，
  還是只是把雜訊切開而已。ISLP §12.4.3 的建議是：換不同設定多跑幾次，
  看哪些結構每次都出現；報告時交代探索方法與尚待驗證的假設。''', "warm")}

{quiz("qWhy", "QUIZ · 為什麼非監督式比較難",
      "把資料分成幾個很緊密的群，為什麼還不能說找到了真實子群？",
      [(True, "群內緊密只符合某個幾何目標，仍要檢查穩定性與領域證據",
        "對。即使資料沒有明確子群，分群演算法仍會給出分組。幾何指標與實質意義是不同的評估目標。"),
       (False, "因為只要沒有 y，就完全不能用留出資料或交叉驗證",
        "不對。只要先定義可評估的目標，就能設計驗證；例如遮住已觀測格子，檢查矩陣補全的補值誤差。這仍不能直接證明群的實質意義。"),
       (False, "因為非監督式方法沒有參數可以調",
        "不對。M、K、linkage、距離與標準化都是要做的選擇；這些選擇都需要先釐清評估目標。")])}
"""
...
BODIES["pca"] = f"""
  ...
  <p>PCA 重新選擇座標軸，尋找<strong>資料變異最大的那個方向</strong>。
  ...
  <strong>負荷向量是新座標軸的方向，得分是每筆資料在新座標軸上的位置</strong>。請區分負荷向量與得分。</p>
...
{card("講義 12 · USArrests 的 PCA", _pca_code, lab_output(CH, 29), src=src("19、21、23、27、29"),
      ...
      note="<code>components_</code> 的<strong>每一列</strong>是一個負荷向量。第一列 "
           ...
           "注意 <code>PCA()</code> 預設只置中、不縮放，所以先用 <code>StandardScaler</code> 標準化，再擬合 PCA。")}
...
BODIES["biplot"] = f"""
  ...
  <ul>
    <li><strong>箭頭的座標 → 變數在兩個主成分中的負荷量。</strong>
    Murder、Assault、Rape 在 PC1 上都有較大的正負荷；UrbanPop 在 PC2 上最突出。
    本圖畫的是原始負荷量，不能直接把箭頭夾角的 cosine 當成相關係數。</li>
  ...
  <p>下面這個 biplot 用的是課本的資料。切換標準化設定，觀察它的影響：
...
{viz(svg("w07biSvg", 400),
     [rows_card("負荷量（箭頭的座標）",
      ...
      info_card("為什麼有些州沒有標字",
                ...
                'IN（兩者都接近平均）、AK。其餘州以點呈現。'
                '<strong>這張 biplot 用來呈現整體結構。</strong>', "圖 12.1")],
...
{card("講義 12 · biplot（scatter + arrow + text）", lab_code(CH, 33), None, src=src("31、33"),
      note="<code>scikit-learn</code> 沒有內建 biplot，所以 lab 用 "
           ...
           "會擠在原點附近）。<strong>整組箭頭同乘一個正的顯示倍率，方向不變。</strong><br>"
...
{quiz("qBi", "QUIZ · 讀 biplot",
      "USArrests 的 PC1 對三個犯罪變數的負荷約為 0.54、0.58、0.54，PC2 對 UrbanPop 的負荷約為 0.87。哪個解讀合理？",
      [(True, "PC1 主要綜合犯罪程度，PC2 的組成以都市化程度最突出",
        "對。箭頭座標呈現各變數在主成分中的係數；這是解讀主成分組成的依據。"),
       (False, "UrbanPop 的原始變異數一定比其他變數大",
        "不對。這張圖使用標準化後的資料，負荷量表示主成分線性組合的係數。原始變異數需回原資料計算。"),
       (False, "只要兩支負荷量箭頭垂直，就能證明兩個原始變數獨立",
        "不對。本圖的箭頭夾角不能直接換成相關係數；即使相關係數為 0，也不代表獨立。")])}
"""
...
{info("實務上怎麼算：SVD", '''求主成分不必真的做特徵分解。把置中後的資料矩陣做
  ...
  $\\mathbf{V}$ 的每一欄就是負荷向量、$\\mathbf{U}\\mathbf{D}$ 就是得分矩陣。
...
{quiz("qLow", "QUIZ · 最佳低維近似",
      ...
      [(True, "把前 M 個主成分的變異最大化，等於把 M 維近似的誤差最小化",
       ...
       (False, "只有在資料標準化過的情況下這個等式才成立",
        ...
        "標準化影響的是主成分本身長什麼樣（P05 會講），不影響這個分解是否成立。")])}
"""
...
{viz(chart("w07screeChart", "tall",
     ...
     [rows_card("滑桿選的 M",
      ...
      info_card("肘點在哪裡？",
                ...
                '<strong>肘點由目測判斷，選擇可能因人而異。</strong>'
...
{quiz("qPve", "QUIZ · PVE 與 scree plot",
      ...
      [(True, "近似的 R² 是 0.868，也就是殘差平方和只剩總平方和的 13.2%",
       ...
       (False, "近似誤差是 0.089 + 0.043 = 0.132 個單位的 MSE",
        "數字對、單位錯。0.132 是<strong>比例</strong>（佔總平方和的 13.2%）；MSE 的絕對值還取決於總平方和。"
        "要換成絕對值得再乘上 TSS。")])}
"""

# ── P05 尺度化與符號 ───────────────────────────────────────────────────
BODIES["scaling"] = f"""
  <p>這一節討論兩個實務問題：<strong>做 PCA 之前要不要標準化</strong>，
...
{card("講義 12 · USArrests 的平均與變異數",
      ...
      note="<strong>6945 對 18.97，差了 366 倍。</strong>看到這種數字就知道非標準化不行了。<br>"
           ...
           "<code>load_data()</code> 所用的 <code>ISLP</code> 資料集中沒有 <code>USArrests</code>，因此改由前述來源讀取。"
...
{qa("觀念釐清", [
    ...
    ("Q：為什麼 PCA 之前幾乎一定要標準化？什麼情況下不該標準化？",
     ...
     "所以這裡先標準化，讓各變數以相同尺度參與分析。</p>"
     "<p>不該標準化的情形：變數同單位、而且變異數的差異本身是你想保留的資訊。"
     "基因表現量、影像的畫素值、同一種感測器的多個通道都屬於這一類。"
...
{info("使用矩陣補全前需檢查的條件", '''<strong>1. 缺失必須是隨機的</strong>（missing at random）。
  「電子秤剛好沒電」可以補；「病人太重上不了秤」不行——缺失本身帶著資訊，
  補出來的值會系統性偏低。<br>
  <strong>2. 變數之間要有相關。</strong>低秩近似能發揮作用是因為欄與欄之間可以互相預測；
  四個互相獨立的變數，補值不會比欄平均好。<br>
  <strong>3. M 要選。</strong>選太小補不出細節，選太大會把雜訊也擬合進去。
...
{card("講義 12 · 挖掉 20 格，然後用主成分反覆填補", _mc_code, lab_output(CH, 64),
      ...
      note="讀法：<code>low_rank(Xhat, M=1)</code> 是步驟 2(a)（用 SVD 取秩一近似）、"
           ...
           "MSS 從 0.395 掉到 0.381 就幾乎不動了，第 8 輪相對誤差低於 <code>1e-7</code> 時停止。"
           "相對改善的分母使用初始值 <code>mss0</code>；<code>mss</code> 是目前值。"
...
{quiz("qMc", "QUIZ · 矩陣補全",
      ...
      [(True, "觀測值是真實資料，是唯一的資訊來源；換掉它們就沒有東西可以把近似「拉住」了",
       ...
       (False, "全部換掉在數學上也對，只是會多算幾輪比較慢",
        "這會改變迭代的目標。把觀測格也覆蓋掉，下一輪的 SVD 就是對"
        "「上一輪的秩 M 近似」做分解。它已經是秩 M 了，下一輪便會停在同一個近似，失去利用觀測格修正補值的作用。"),
...
BODIES["kmeans"] = f"""
  ...
  <p>右邊只需要算每個點<strong>到群心的距離</strong>。這個恆等式能減少計算量，
  也說明了演算法為何反覆更新群心與各點的分組：</p>
...
{card("講義 12 · K = 2 的模擬資料", _km_code, lab_output(CH, 107), src=src("103、105、107"),
      ...
      note="資料是刻意造的：前 25 筆的平均被平移過，所以真的有兩群。"
           ...
           "<strong>群的編號是任意的</strong>，0 與 1 交換後仍是相同的分組。"
...
{card("講義 12 · n_init 為什麼要設大", lab_code(CH, 113), lab_output(CH, 113), src=src("113"),
      note="<code>inertia_</code> 就是群內平方和（式 12.17 要最小化的那個）。"
           ...
           "差別只在試了幾組初始值。<br>"
           "<strong>76.85 是演算法收斂到的一個局部極小</strong>，程式不會警告你。"
...
{quiz("qKm", "QUIZ · K-means",
      ...
      [(True, "步驟 2(a) 用群內平均使平方偏差最小、步驟 2(b) 把點移到更近的形心，兩步都不會讓目標變大",
       ...
       (False, "因為每一步都會減少群數，群數減到 K 就停了",
        ...
        "下一節的階層式分群會逐步合併群；K-means 則全程維持 K 群。"),
       (False, "因為 scikit-learn 設了 max_iter 上限，跑到上限就停",
        "<code>max_iter</code> 提供迭代次數上限；停止的數學理由是收斂。"
...
BODIES["hclust"] = f"""
  ...
  <p>凝聚式（agglomerative，也叫 bottom-up）的做法如下：</p>
...
{info("演算法 12.3（階層式分群）", '''<strong>步驟 1：</strong>每一筆資料自己是一群，
  ...
  <p>讀樹狀圖（dendrogram）時，要區分合併高度與水平位置：</p>

{info("用合併高度讀樹狀圖", '''兩筆資料有多相似，看的是<strong>它們所在的分支第一次合併的高度</strong>，
  水平方向的距離由排版決定。<br>
...
{viz(svg("w07dendroSvg", 350),
     [rows_card("這一刀切出什麼",
      ...
      info_card("怎麼玩",
                ...
                '往上切群數變少、往下切變多。<br>'
                '用下拉選單比較：把 linkage 換成 <strong>single</strong>，'
                '觀察樹狀圖如何變成鏈狀。'),
     ...
     "w07dendroStatus", "拖動橘色切線改變群數，用下拉選單比較四種 linkage。",
...
BODIES["practical"] = f"""
  <p>實際分群時，前處理與參數選擇都會影響結果。ISLP §12.4.3 把它們列成一張清單，
  每一項都會實質改變結果：</p>
  ...
  <p>下面這個元件沿用課本圖 12.16 的「襪子與電腦」情境；課本提供概念，逐筆數值另作示意設定。
...
{viz(svg("w07practSvg", 340),
     [rows_card("K = 2 的結果",
      ...
      info_card("課本第 5 題就是這一題",
                ...
                '描述你預期看到什麼。」用上方選單切換三種尺度，就能比較結果。', "第 5 題")],
  ...
  <p>沒有一個通用指標能直接證明群具有實質意義。
  <strong>任何時候把資料丟去分群，它都會給你群</strong>——即使資料是純雜訊。
  要檢查的是「這些群在獨立的新資料上也會出現嗎」，
  文獻提出過不同的分群 p 值方法，其適用方式仍有爭議（細節在 ESL）。</p>
...
{table(["做法", "怎麼做", "在檢查什麼"],
       [["換設定重跑", "換 linkage、換距離、換 K、標準化與否",
        ...
        ["對照外部標籤", "有領域標籤時用 <code>crosstab</code> 或 ARI 比對",
         "分群有沒有抓到已知的結構（限於<em>事後</em>檢查，避免用同一標籤調參）"],
...
{card("講義 12 · NCI60 上兩種分群的交叉表", _vs_code, lab_output(CH, 176),
      ...
      note="同一份 NCI60（64 個細胞株 × 6830 個基因）、同樣切 4 群，"
           "兩種方法的結果<strong>略有不同</strong>："
           "K-means 的第 3 群等於階層式的第 2 群，但 K-means 的第 0 群"
           "混了階層式第 0 群的一部分加上整個第 1 群。<br>"
           "先看群編號是任意的（可用 <code>crosstab</code> 對照群的成員；直接比較編號會受任意命名影響）。"
           "lab 儲存格 172 另外把階層式的 4 群對上真實癌症類型："
           "所有白血病落在同一群，但乳癌散在三群——"
           "<strong>分群抓到了部分結構，仍有癌症類型分散在不同群中。</strong>")}

{qa("觀念釐清", [
    ("Q：分群結果「對不對」要怎麼判斷？沒有 y 的時候有沒有客觀標準？",
     "<p>沒有一個通用數字能直接證明群是真的。沒有已知群標籤時，不能直接算分類錯誤率；"
     "但仍可先訂評估目標，再比較幾何結構、穩定性或留出資料上的表現。</p>"
     "<p>常見的內部指標（silhouette、Calinski–Harabasz、gap statistic）能算，"
     "這些指標量的是<strong>幾何上的緊密與分離</strong>；群的實質意義仍需外部證據。"
     ...
     "ISLP 將分群結果定位為<strong>形成科學假設的起點</strong>，仍需後續證據檢驗。</p>"),
    ("Q：PCA 與分群都在「找結構」，差在哪？",
...
{quiz("qPrac", "QUIZ · 分群的實務決策",
      ...
      [(True, "兩種方法都會被離群值扭曲，因為它們強迫每一點都進某一群；"
       ...
       (False, "階層式分群不受影響，因為離群值會自己形成一個單獨的分支",
        ...
        "原本要觀察的結構可能被壓在一起。single linkage 更糟，離群值會把整棵樹拉成鏈狀。"),
...
BODIES["manifold"] = f"""
  ...
  <a href="#exercises">EX 練習</a>。t-SNE 在論文裡到處都是，解讀前應先了解它的限制。</p>
...
{viz(chart("w07tsneChart", "tall",
           ...
           "t-SNE 的投影呈現清楚的團，解讀仍應限於局部鄰近關係；團的大小與團間距離會受投影影響。"),
     [rows_card("目前這張圖",
...
{quiz("qEx3", "EXERCISE 3 · ISLP §12.6 第 5 題",
      ...
      [(True, "原始次數 → 由襪子決定；標準化 → 變成「有買電腦 / 沒買電腦」；"
       ...
       (False, "三種尺度會給同樣的分群，因為 K-means 對線性變換是不變的",
        ...
        "線性迴歸的擬合值具有這種尺度不變性；K-means 使用的距離會受尺度影響。"),
       (False, "標準化之後電腦的影響會消失，因為 0/1 變數標準化後變異數變成 1 太小",
        "方向剛好相反。標準化把兩個變數的變異數都設成 1，"
        "電腦是從「幾乎沒有影響」變成「影響力跟襪子相同」，仍然參與分群。")])}
...
{quiz("qEx4", "EXERCISE 4 · ISLP §12.6 第 8 題",
      ...
      [(True, "兩邊必須用<strong>同一份</strong>資料：(a) 用標準化後的資料跑 PCA，"
       ...
       (False, "警告要用 ddof=1 算變異數，否則兩邊差一個 n/(n−1) 的因子",
        ...
        "差異在於兩邊<strong>用了不同的資料</strong>（一邊標準化、一邊沒有）。")])}
"""
...
{table(["主成分", "PC1", "PC2", "PC3", "PC4"],
  ...
  18.97 / 6945.17 / 209.52 / 87.73（儲存格 17），因此本例先標準化，讓各變數以相同尺度參與。</p>
...
<h3>Mean shift、混合模型與譜分群</h3><p>Mean shift 把候選中心移到鄰近點的加權平均，反覆向核密度的高處移動；收斂到相近模態的起點併為一群。頻寬決定平滑程度與可辨識的模態數，不必預先指定 K，卻仍需選頻寬。高斯混合模型以多個高斯成分的加權密度表示資料，EM 交替更新各點屬於各群的後驗機率（責任值）與參數，允許軟分群；一般高斯混合仍會為每個點計算各群的責任值，若要處理離群值需另外設計雜訊成分或穩健模型。變分貝氏混合在成分權重與參數上加入先驗，以近似後驗估計不確定性。</p><p>譜分群（spectral clustering）先建立相似度圖，利用圖 Laplacian 的特徵向量嵌入，再於嵌入座標分群。鄰接圖、核尺度與群數會改變答案；它與 DBSCAN 都能處理某些非球狀群，但一般並非同一演算法。</p>
<h3>混合型資料與分群後的用途</h3><p>名目類別不可任意編號後直接套歐氏距離。可只用連續特徵、對類別做適當編碼並控制權重、使用結合數值距離與類別不一致的成本的 k-prototypes，或先用混合資料因素分析得到連續座標。這些選擇代表不同的「相似」定義，需要連同結果報告。</p><p>分群標籤、到各群中心的距離及特徵聚合都能當作監督式模型的輸入；群標籤本身通常應視為類別。評估時分群也要只在訓練折擬合。少量標記時，可優先標註群中心附近的代表樣本，再在群內傳播標籤；這依賴群大致對應真實類別的假設，群跨越類別時會放大錯誤。</p>
"""
BODIES['manifold'] += r"""
...
function w07practDraw() {
  ...
  const msg = {
    raw: '用原始次數：襪子的變異數比電腦大幾十倍，歐氏距離幾乎只由襪子決定，'
       + '兩群就是「買比較多襪子」與「買比較少襪子」——電腦有沒有買在這個分組中沒有發揮作用。',
...
PAGEJS += apply_reading_flow('unsupervised_learning', BODIES)
from teaching_scope_ch7_8_9_12 import clean_pagejs
PAGEJS = clean_pagejs('unsupervised_learning', PAGEJS)
