...
BODIES["bootstrap"] = f"""
  ...
  <p class="core-backfill">抽樣分佈與自助法還不熟時，可先回補
...
迴歸曲線則在每次重抽觀測對並重新配適後，用固定 x 網格收集預測，再逐點取分位數。
逐點區間不是整條曲線的同時信賴帶；估計偏差、樣本太少或統計量不規則時，簡單 percentile 法的涵蓋率也可能不足。</p>
<p><strong>預測區間</strong>還須包含新觀測的隨機誤差。在獨立、同變異數的迴歸設定，可在每次重配的均值預測上
加入從適當中心化殘差抽到的新誤差，再取預測分位數。只重抽係數或均值曲線會漏掉這一項。
...
{info("三個一定要記住的觀念", '''<strong>1. 交叉驗證估「預測誤差」，bootstrap 估「估計量的不確定性」。</strong>
  ...
  <strong>3. 任何從資料估參數的前處理都要關在折裡面。</strong>
