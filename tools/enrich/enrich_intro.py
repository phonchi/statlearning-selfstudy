...
{quiz("qRegCls", "QUIZ · 迴歸與分類",
      ...
      [(True, "看你怎麼定義輸出——輸出機率值就是迴歸；輸出「會下／不會下」就是分類",
        "對。同一個現實問題可以包裝成兩種。實務上常見的做法是<strong>用分類模型輸出機率</strong>（邏輯斯迴歸就是這樣），再選一個閾值變成硬決策——第 4 章的閾值滑桿元件在演這件事。"),
       (False, "一定是迴歸，因為機率是 0 到 1 之間的連續數值",
        "只答對一半。輸出連續值確實是迴歸的形式，但這個問題的<em>原始</em> y 是「有沒有下雨」這個二元事件，用分類模型建模再輸出機率通常更自然也更準。"),
       (False, "一定是分類，因為下雨只有兩種可能",
        "也只答對一半。若你要回報的是機率而不是硬決策，那個輸出是連續的。關鍵在於<strong>你要交出什麼</strong>，不是現象本身有幾種狀態。")])}
"""
...
{info("p ≫ n 會出大事", '''NCI60 資料是 <strong>64 × 6830</strong>——變數比觀測值多一百倍。
  ...
  但那個模型毫無預測能力。第 6 章有一整節在講這件事，
  以及該用什麼方法（收縮、降維）才活得下來。''', "warm")}
...
{quiz("qEx1", "EXERCISE 1 · 分類問題的四個象限",
      ...
      [(True, "監督式、分類、偏預測 → 第 4 章（分類）與第 8、9 章（彈性方法）",
       ...
       (False, "監督式、迴歸，因為要預測的是機率",
        "如果你要輸出機率，形式上確實是連續值——但原始的 y 是二元事件，標準做法是用分類模型（邏輯斯迴歸）輸出機率。歸類為分類問題才對得上第 4 章的工具。")])}
...
{quiz("qEx3", "EXERCISE 3 · n 與 p",
      ...
      [(True, "n = 200、p = 20000；p ≫ n，最小平方會完美配適訓練資料但毫無預測力",
        "對。這是第 6 章「高維度」那一節的核心情境。p ≥ n 時最小平方能讓訓練 RSS = 0、R² = 1，那只反映自由度用完了。要用收縮（Lasso、Ridge）或降維（PCR、PLS）才活得下來。"),
       ...
       (False, "n = 200、p = 20000；資料量夠大，用隨機森林就沒問題",
        "n 與 p 對了，但「沒問題」錯了。p ≫ n 對所有方法都是挑戰——隨機森林比較耐受，但仍然會被大量無關變數拖累（第 6 章的維度詛咒模擬，候選變數從 20 加到 2000 時測試誤差從 1.34 升到 7.54）。")])}
