...
{qa("觀念釐清", [
    ("p 值 0.000 是不是代表這個變數很重要？",
     "不是。在模型假設與零係數假設成立時，p 值衡量出現目前或更極端統計量的機率。小 p 值提供反對零係數假設的證據，"
...
{viz(svg("w19flowSvg", 340),
     [info_card("三個動詞",
                "按「單步」看資料怎麼流過去。注意 <code>fit</code> 用訓練資料的 y 學參數；"
                "<code>predict</code> 只吃 X，<code>score</code> 則需要 y 作為評分答案。"),
      rows_card("目前",
                [("步驟", "0 / 3", "w19flStep"),
                 ("這一步做什麼", "—", "w19flWhat"),
                 ("學到的東西存在哪", "—", "w19flWhere")]),
...
const w19smInfo = [
  ...
  {w: '「係數其實是 0」的話有多難得到這個結果', n: '不要只看這一欄',
   note: '小 p 值是在模型假設下反對零係數假設的證據，<b>跟效果大不大無關</b>。'}
];
let w19smI = 0;
function w19smDraw() {
