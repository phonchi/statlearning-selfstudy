...
BODIES["prologue"] = f"""
  ...
  你能逐行<strong>讀懂課程 lab 的程式</strong>。後面幾頁再練習自己寫。</p>

{info("先從三件事讀程式", "① <strong>算</strong>一個值；② 把值<strong>取個名字</strong>存起來；"
      "③ 把值<strong>丟給某個函式</strong>處理。"
      "閱讀課程 lab 時，先找出這三件事如何組合。")}
...
# ── P01 變數與型別 ─────────────────────────────────────────────────────
BODIES["var"] = f"""
  <p>可以把變數想成物件上的名牌。<code>x = [3, 4, 5]</code> 會先做出一個串列，
  然後把 <strong>x 這個名牌貼上去</strong>。同一個東西可以貼很多張名牌——
  P3 會用陣列的「檢視」示範共用資料時的修改結果。</p>

{viz(svg("w14varSvg", 320),
     [info_card("兩個名字指向同一個物件",
...
# ── P04 字典 ────────────────────────────────────────────────────────────
BODIES["dict"] = f"""
  <p>串列用位置取值，字典用<strong>名字</strong>取值。統計程式常用字典儲存「一組具名的結果」：模型的評分、欄名對應到型別、參數名稱對應到值。</p>
...
{viz(svg("w14dcSvg", 300),
     [info_card("兩種取值方式",
                "左邊是串列（用位置），右邊是字典（用名字）。"
                "按按鈕看同一個查詢在兩邊怎麼寫，並比較用名字與位置取值時需要確認的資訊。"),
      rows_card("目前",
                [("要拿什麼", "—", "w14dcWant"),
                 ("串列寫法", "—", "w14dcList"),
                 ("字典寫法", "—", "w14dcDict")]),
...
{quiz("qDict", "PART 04 · 自我檢測",
      ...
      [(True, "拋 <code>KeyError</code>",
       ...
       (False, "自動建一個空的 MSE 項目",
        "讀取不會建立。<code>scores['MSE'] = 38.5</code> 這種<strong>指派</strong>才會。")])}
"""
...
{info("三個閱讀重點",
      ...
      "有名字的資料可以用字典儲存，省去記「第 2 個是 MSE」這類位置對應。")}
