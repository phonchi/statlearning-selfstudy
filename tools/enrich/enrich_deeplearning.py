...
BODIES["prologue"] = f"""
  ...
  換基底 → 集成」這條線走完之後，深度學習正好是同一條線的下一步，可以銜接前面的內容。</p>
...
{info("這一章的六個主題", '''<strong>1. 單層網路（§10.1）：</strong>一層隱藏層，非線性從哪裡進來。<br>
  <strong>2. 多層與 MNIST（§10.2）：</strong>疊第二層、softmax 輸出、參數量快速增加。<br>
  <strong>3. 卷積網路（§10.3）：</strong>權重共享與池化，把影像的結構寫進架構裡。<br>
  <strong>4. 文件分類與 RNN（§10.4–10.5）：</strong>順序有意義的時候，詞袋不夠用。<br>
  <strong>5. 怎麼擬合（§10.7）：</strong>反向傳播、SGD、dropout。<br>
...
BODIES["multi"] = f"""
  ...
  <p>MNIST 是十類別的問題，所以輸出層要輸出十個數字再轉成機率。用的是
  <strong>softmax</strong>：</p>

  $$f_m(X) = \\Pr(Y = m \\mid X)
    = \\frac{{e^{{Z_m}}}}{{\\sum_{{\\ell=0}}^{{9}} e^{{Z_\\ell}}}} \\tag{{10.13}}$$

  <p>損失函數則是負的多項式對數概似，也就是<strong>交叉熵</strong>：</p>
...
{info("softmax 是過度參數化的", '''把每個 Z_m 同時加上一個常數 c，分子分母各多一個 e^c，
  ...
  <p>參數量是這一節的重點。MNIST 的輸入是 28×28 = 784 個畫素，
...
{card("lab §10.9.2 · MNISTModel 與參數量",
      ...
      note="兩層的 dropout 比例不一樣（0.4 與 0.3）：靠近輸入的層通常丟得多一點。"
           ...
           "<strong>這個動作把畫素的空間關係整個丟掉了</strong>，"
           "下一節的卷積網路就是為了把它找回來。")}
...
# ── P03 cnn ───────────────────────────────────────────────────────────
BODIES["cnn"] = f"""
  <p>上一節把 28×28 攤平成 784 的那一刻，「這兩個畫素相鄰」的資訊就沒了。
...
{viz(svg("w11convSvg", 420),
     [info_card("怎麼玩",
      ...
      info_card("為什麼參數少這麼多",
                ...
                '那就是習題 4(d) 在算的東西。')],
     "w11convSvgStatus", "拖列與欄，直接檢查任一卷積視窗和對應輸出。",
     ...
     '<label class="slider-label">欄</label><input id="w11convCol" type="range" min="1" max="6" value="1" oninput="w11convSelect()">'
...
{qa("觀念釐清", [
    ("Q：池化為什麼要「丟掉資訊」？這樣有什麼好處？",
     ...
     "你要判斷這是不是一隻貓，不需要知道耳朵精確在第 137 個畫素。</p>"
...
{card("lab §10.9.3 · 一個 building block 疊四次",
      ...
      note="<code>BuildingBlock</code> 就是「卷積 → ReLU → 最大池化」三個步驟。"
...
{quiz("qConv", "QUIZ · 卷積的參數量",
      ...
      [(True, "78 個：3 × (5×5 + 1)",
       ...
       (False, "2,408,448 個，每個隱藏單元都要連到全部 1,024 個畫素",
...
BODIES["rnn"] = f"""
  ...
  ISLP 用 IMDB 影評的正負評分類當例子，先示範最簡單的做法：<strong>詞袋</strong>。
...
{info("架構與模型表現",
      ...
      "詞袋實跑結果比較。公式說明架構，lab 結果提供這次擬合的表現。")}
...
{quiz("qSeq", "QUIZ · 詞袋輸給 RNN 了嗎",
      ...
      [(True, "這個任務的訊號多半在「有沒有出現某些詞」，詞序能加的資訊有限，"
       ...
       (False, "因為 LSTM 只訓練了很少的 epoch，訓練久一點就會贏",
        "沒有證據支持這個推測，而且它把問題想反了：訓練更久通常先過度擬合。"
...
_dd_controls = (
    ...
    '<button class="btn btn-toggle" onclick="w11ddShow(&quot;fit&quot;)">看擬合曲線</button>'
...
BODIES["doubledesc"] = f"""
  ...
  然後擬合自由度 d 的自然樣條，讓 d 從 2 一路加到 100。</p>
...
{viz(chart("w11ddChart", "tall",
     ...
     [info_card("怎麼看這張圖",
                ...
                '按「看擬合曲線」切換到課本圖 10.21 的四個面板。', "圖 10.20／10.21"),
      rows_card("幾個位置",
                [("d = 8（U 形谷底）", "—", "w11dd8"),
                 ("d = 19", "—", "w11dd19"),
                 ("d = 20（內插門檻）", "—", "w11dd20"),
...
{info("雙下降沒有推翻偏差–變異取捨", '''ISLP 提醒讀者：橫軸畫的是「基底函數的個數」，
  ...
  如果改用嶺迴歸擬合樣條並好好選懲罰項，測試誤差還會更低。
  <strong>判斷模型大小時，仍需比較正則化與測試誤差。</strong>''')}

  <p>這跟深度學習的關係在最後一段：訓練神經網路時我們常常用遠多於樣本數的參數，
  而且真的能把訓練誤差壓到 0 還表現得不錯。ISLP 的解釋是
  <strong>最佳化方法的隱含偏好也會影響最後選到的內插解</strong>。例如平方損失的線性模型從零初始化，採適當步長的梯度下降收斂時可得到最小歐氏範數解；一般神經網路的 SGD 不保證得到最小範數或最平滑解。
  是否泛化良好仍要由資料、模型、正則化與獨立評估判斷。</p>
...
{quiz("qDd", "QUIZ · 雙下降在說什麼",
      ...
      [(True, "測試誤差的第二次下降，是因為 d > n 時我們挑了最小範數解，"
        ...
        "在這個樣條基底與資料設定下，最小範數選解呈現較平滑的曲線；滿秩的 d = n 系統只有唯一內插解。最小係數範數依賴基底，不是所有函數空間都保證最小曲率。"),
       (False, "偏差–變異取捨在深度學習裡不成立，模型愈大測試誤差就愈低",
        "ISLP 明確反駁了這個讀法。取捨仍然成立，只是橫軸那個「基底個數」"
        "沒有正確衡量彈性。而且課本自己說了，同一個例子改用嶺迴歸會得到"
        "<strong>更低</strong>的測試誤差。增加模型大小仍有擬合與泛化的取捨。"),
...
{quiz("qEx3", "EXERCISE 3 · ISLP 10.10 第 4 題（b）（d）",
      ...
      [(True, "78 個與 2,408,448 個",
        ...
        "個隱藏單元（32 − 5 + 1 = 28），每個都要連到全部 32 × 32 = 1,024 個畫素，"
...
{info("重點回顧", '''<strong>1. 神經網路是線性模型套非線性再組合一次。</strong>
  ...
  <strong>2. CNN 與 RNN 都使用權重共享。</strong>
...
function w11convApply(f) {
  ...
  $('w11convPos').textContent = '第 ' + (rr + 1) + ' 列、第 ' + (cc + 1) + ' 欄';
  ...
  if (mode === 'err') {
    ...
    const wild = d === '20' ? '——<b>它是唯一能穿過 20 個點的樣條，所以只能長這樣</b>'
      : (parseInt(d, 10) > 20
        ...
        : '——自由度不夠，它沒有穿過每一個點');
    setStatus('w11ddStatus',
      'd = ' + d + ' 的擬合曲線，係數平方和 Σβ² = ' + fit.l2 + wild + '。'
