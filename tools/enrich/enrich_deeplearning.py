...
# ── P00 prologue ──────────────────────────────────────────────────────
BODIES["prologue"] = f"""
  <p>先講清楚這一章的定位：<strong>本課沒有教第 10 章</strong>。它不在考試範圍，
  也沒有對應的講義與 lab。放上來是因為前面九章把「線性模型 → 加彈性 → 加正則化 →
  換基底 → 集成」這條線走完之後，深度學習正好是同一條線的下一步，缺了它整本書會斷掉。</p>

  <p>ISLP 用一個很不客氣的方式介紹神經網路：把式子攤開來看，它就是
...
{info("g 的兩個常見選擇", '''<strong>Sigmoid：</strong>g(z) = eᶻ/(1+eᶻ)，把任何實數壓進 (0, 1)。
  ...
  <strong>ReLU：</strong>g(z) = max(0, z)。負的歸零、正的原樣過。導數不是 0 就是 1，
  算得快也不會消失，現代網路的預設。''')}
...
{qa("觀念釐清", [
    ("Q：隱藏單元的個數 K 該怎麼選？",
     ...
     "用交叉驗證或驗證集選。</p>"
     "<p>但實務上的取法跟你想的可能不一樣：<strong>現代做法是先把 K 開大，"
     "再靠正則化（dropout、權重衰減、早停）把彈性收回來</strong>，"
     "而不是小心翼翼地找剛剛好的 K。原因在 §10.8：過度參數化的網路配上"
...
{card("lab §10.9.1 · 訓練 50 個 epoch 之後測試",
      ...
      note="<code>test_mae</code> = <strong>221.83</strong>，比 lasso 的 235.68 好。"
           ...
           "換個切分與 epoch 數就換了名次，這件事本身比誰贏更值得記住。")}
...
{quiz("qHit", "QUIZ · 怎麼讀這組數字",
      ...
      [(True, "兩邊是不同的隨機切分與訓練設定，這種量級的差距落在單一測試集的變異裡，"
        ...
        "不是看一次 hold-out。ISLP 自己的結論是三個模型「表現相近」，然後選簡單的那個。"),
...
# ── P02 multi ─────────────────────────────────────────────────────────
BODIES["multi"] = f"""
  <p>多加一層之後，第二層的輸入不再是 X，而是第一層的輸出 A^(1)。
  式子還是那個式子，只是套了兩次：</p>
...
{quiz("qParam", "QUIZ · 參數量",
      ...
      [(True, "200,960 個：權重 784×256 = 200,704，加上 256 個偏置",
       ...
       (False, "200,704 個，偏置不算參數因為它不乘任何輸入",
        "偏置也要估，當然算參數。它就是迴歸裡的截距，你不會說線性迴歸的 β₀ 不是參數。"
        "少算 256 個雖然不影響量級，但習題 1(d) 考的正是這個細節。"),
       (False, "1,040 個：784 + 256 個單元各一個參數",
        "把「單元個數」當成「參數個數」了。參數在<strong>連線</strong>上，不在單元上："
        "784 個輸入各自連到 256 個隱藏單元，連線數是相乘不是相加。")])}
"""
...
{qa("觀念釐清", [
    ...
    ("Q：資料擴增算正則化嗎？",
     ...
     "<p>差別在於它是<strong>用資料表達</strong>的約束，不是用公式。"
...
{quiz("qConv", "QUIZ · 卷積的參數量",
      ...
      [(True, "78 個：3 × (5×5 + 1)",
       ...
       (False, "2,352 個，因為輸出有 3 × 28 × 28 個隱藏單元",
        "那是<strong>單元</strong>個數不是<strong>參數</strong>個數。"
...
BODIES["rnn"] = f"""
  ...
  聽起來很糟，實際上在情感分類這種任務上表現意外地好，因為有沒有出現
...
{quiz("qSeq", "QUIZ · 詞袋輸給 RNN 了嗎",
      ...
      [(True, "這個任務的訊號多半在「有沒有出現某些詞」，詞序能加的資訊有限，"
       ...
       (False, "LSTM 比較差，代表遞迴架構不適合處理文字",
        "從一個任務、一次訓練就推翻整個架構，太快了。"
        "RNN 與其後繼者在機器翻譯、語音辨識上的優勢很清楚："
        "那些任務的輸出本身就是序列，詞序不是加分項而是必要條件。"),
...
BODIES["fitting"] = f"""
  ...
  接受「找到一個夠好的解」而不是「找到最好的解」。</p>

{viz(chart("w11gdChart", "tall",
           "。此圖的重點：同一個函數、同一個學習率，只是起點不同，"
           "梯度下降就會滑進不同的局部極小，這就是非凸最佳化的日常。"),
     [info_card("怎麼玩",
...
{qa("觀念釐清", [
    ("Q：非凸為什麼還敢用？局部極小不會害死我們嗎？",
     "<p>會，但沒有想像中嚴重，原因有幾個層次。</p>"
     "<p>第一，實務上在高維空間裡，真正的「壞局部極小」比想像中少："
     "更常見的是鞍點，而 SGD 的雜訊很容易把你推離鞍點。"
     "第二，我們要的不是訓練誤差的全域最小，而是<strong>測試</strong>誤差夠低；"
...
BODIES["doubledesc"] = f"""
  ...
  所以不要把訓練誤差壓到 0。§10.8 要示範一個看起來完全相反的現象。</p>
...
{info("雙下降沒有推翻偏差–變異取捨", '''ISLP 把話講得很白：橫軸畫的是「基底函數的個數」，
  ...
  <strong>雙下降不是「參數愈多愈好」的許可證。</strong>''')}
  ...
  但反過來，CIFAR 的影像與 IMDB 的文字這種輸入本身有結構、樣本量又大的問題，
  線性模型從一開始就沒有機會。</p>

{quiz("qDd", "QUIZ · 雙下降在說什麼",
      "下面哪一句是 ISLP §10.8 真正主張的？",
      [(True, "測試誤差的第二次下降，是因為 d > n 時我們挑了最小範數解，"
        "而它比 d = n 的唯一內插解平滑得多",
        "對。關鍵不在「參數變多」，而在<strong>參數變多之後我們有得挑</strong>，"
...
{quiz("qEx1", "EXERCISE 1 · ISLP 10.10 第 1 題（d）",
      ...
      [(True, "23 個",
       ...
       (False, "10 個",
        ...
        "參數在<strong>連線</strong>上不在單元上：單元只是中間量，"
        "它的值是算出來的，不需要估。")])}
...
{quiz("qEx2", "EXERCISE 2 · ISLP 10.10 第 2 題（a）",
      ...
      [(True, "完全不變，因為分子分母各多出一個 e^c，約掉了",
       ...
       (False, "機率會變得更平均，因為指數函數把大的值壓縮了",
        "「加常數」跟「壓縮」是兩回事。真正會讓機率變平均的是把所有 Z_ℓ "
...
{quiz("qEx4", "EXERCISE 4 · ISLP 10.10 第 5 題",
      ...
      [(True, "兩個指標對大誤差的懲罰方式不同：R² 建立在平方誤差上，"
        ...
        "對平方誤差的貢獻卻是四倍。所以<strong>「多數點準、少數點爆掉」的模型 MAE 會贏、"
...
# ── REF ───────────────────────────────────────────────────────────────
BODIES["reference"] = f"""
  <p>考前把這一頁掃過去就好。這一章不在考試範圍，但它是 ISLP 的完整拼圖。</p>
...
{info("三個一定要記住的觀念", '''<strong>1. 神經網路是線性模型套非線性再組合一次。</strong>
  把 g 拿掉就退回線性迴歸；非線性只能從 g 進來，加總本身不會產生它。<br>
  <strong>2. 權重共享是 CNN 與 RNN 的共同心臟。</strong>
  一個在空間上共享、一個在時間上共享，兩者都是把「結構」寫進架構而不是靠資料學。<br>
  <strong>3. 雙下降不是「參數愈多愈好」。</strong>
  第二次下降來自最小範數解比較平滑，不是來自參數變多本身；
  同一個例子改用嶺迴歸還會更好。''')}

{info("順手提醒", '''<strong>先跑基準再跑網路。</strong>ISLP 每個例子都先給線性模型或 lasso 的數字，
...
function w11gdRun() {
  ...
  if (near === w11gdMIN[1]) {
    ...
    msg += ' <b>還沒收斂——學習率太大，它在跳來跳去而不是滑下去。</b>'
      + '把 ρ 調小一點再試一次。';
  }
...
function w11ddShow(mode) {
  ...
  if (mode === 'err') {
    ...
    HC.line('w11ddChart', {
      datasets: [
        { label: '真正的 f(X) = sin(X)', data: F.grid.map((x, i) => ({ x: x, y: F.truth[i] })),
    ...
    setStatus('w11ddStatus',
      ...
      + '切到 d = 20 再切到 d = 42，看那條橘線怎麼從狂震變回溫和。');
  }
}
function w11ddInit() {
  const F = FRAMES_w11dd;
