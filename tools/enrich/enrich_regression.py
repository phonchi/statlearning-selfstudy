...
BODIES["inference"] = f"""
  ...
  <strong>噪音 σ² 小、樣本多、x 散得開</strong>。σ² 通常不知道，
...
{quiz("qR2", "QUIZ · R² 與 RSE",
      ...
      [(True, "R² 一定上升（或不變），RSE 通常上升",
       ...
       (False, "R² 與 RSE 都會變差，因為變數沒有用",
        "R² 那半錯了。「沒有用」是<em>母體</em>層次的話；在<em>這一份</em>樣本上，最小平方法一定能靠它多刮掉一點 RSS（哪怕只是配到噪音），所以訓練 R² 一定上升。"),
       (False, "兩個都不變，因為最小平方法會自動把它的係數設成 0",
        "不對。最小平方法沒有這種機制，它會給出一個剛好最貼合這份樣本噪音的非零係數。「把沒用的係數壓成 0」要等到第 6 章的 lasso。")])}
"""
...
{table(["Advertising 加入 TV×radio（ISLP 表 3.9）", "係數", "SE", "<em>t</em>", "p 值"],
  ...
  <strong>96.8%</strong>。換個算法更有感：加法模型配完後<em>剩下</em>的變異裡，
...
{quiz("qEx3", "EXERCISE 3 · ISLP 3.7 第 4 題（a）（b）",
      ...
      [(True, "訓練 RSS：三次一定較低（或相等）；測試 RSS：預期線性較低",
       ...
       (False, "兩個都是三次較低，因為三次模型比較有彈性",
        "訓練那半對，測試那半錯。彈性在訓練資料上永遠是優勢，在測試資料上只有「真相真的需要那個彈性」時才是優勢。這裡真相是線性的，多出來的彈性只是在配噪音。"),
       (False, "資訊不足，兩個都無法判斷",
...
function w03knnDraw() {
  ...
  if (w03knnV === 0) {
    ...
    setStatus('w03knnStatus', '真相強烈非線性時，KNN 在 K = ' + S.bestK + ' 附近最好（MSE '
      ...
      + '。兩端都變差：K 太小配到噪音、K 太大把結構糊掉——又是偏差與變異的取捨。');
  } else {
    HC.line('w03knnChart', {
    ...
    setStatus('w03knnStatus', 'p 從 1 到 20：線性迴歸只從 ' + HC.fmt(F.dim['1'].lin, 3)
      ...
      + '——多出來的變數全是純噪音，卻直接污染了「誰是鄰居」的計算。這就是維度詛咒。');
  }
}
