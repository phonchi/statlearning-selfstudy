...
BODIES["inference"] = f"""
  ...
  <p class="core-backfill">標準誤與區間還不熟時，可先回補
...
{qa("觀念釐清", [
    ("Q：線性迴歸的 LINE 假設，到底假設了什麼？",
     ...
     "才能說誤差相關或異質變異主要傷到傳統 SE，而不必然傷到係數。</p>"),
])}
...
{quiz("qInt", "QUIZ · 質性變數與交互作用",
      ...
      [(True, "個別虛擬欄的係數、SE、p 值都會變；配適值、預測、R²、整體 F 檢定都不變",
       ...
       (False, "什麼都不會變，因為虛擬變數的編碼方式不影響模型",
        "<strong>配適</strong>不受影響；<strong>係數的意義</strong>則隨比較對象改變：它們是「相對於基準」的差距，換了基準就是換了比較對象，數字當然不同。"),
       (False, "係數不變，但 R² 會變，因為模型矩陣不同了",
        "兩個模型矩陣張出同一個線性空間，所以配適值與 R² 完全相同；換基準會改變係數的比較對象，因此係數會變。")])}
"""
...
{quiz("qDiag", "QUIZ · 診斷圖",
      ...
      [(True, "線性假設被違反了，係數本身的解讀就已經有問題，要先修模型的形狀",
        "對。這表示目前的線性條件平均形狀不合適。若 E[ε｜X] = 0 仍成立，誤差相關或異質變異主要使傳統 SE 與檢定失準；若外生性也失敗，係數同樣可能偏掉。修法是加 x²、log x，或第 7 章的樣條。ISLP 圖 3.9 左就是這個病徵，加了 horsepower² 之後 U 形就消失了。"),
...
{table(["", "線性迴歸", "KNN 迴歸"],
       [["對 f 的假設", "線性（強）", "只假設平滑（弱）"],
        ...
        ["真相很彎時", "系統性偏差，救不回來", "<strong>表現明顯較好</strong>"],
