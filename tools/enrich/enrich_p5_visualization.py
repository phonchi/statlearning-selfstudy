...
{table(["圖", "高度／位置代表什麼", "適合回答"],
  ...
  <p>比較兩組平均時，要看兩組<strong>差值</strong>的不確定性。各組自己的信賴區間即使重疊，
  差值的區間仍可能不含 0；是否能下結論還取決於樣本數、抽樣方式與模型條件。
  可回<a href="s4_inference.html#intervals">S4 的信賴區間</a>確認區間在估哪一個量。</p>

{quiz("qCat", "PART 04 · 自我檢測",
      "一張 <code>barplot</code> 顯示週四的小費比週五高。你還需要知道什麼才能下結論？",
      [(False, "不用，長條比較高就是比較高",
        "長條的高度是<strong>平均</strong>。兩組平均有差，不代表這個差站得住腳。"),
       (True, "兩組樣本數、誤差線的定義，以及兩組平均差的區間",
        "對。<code>barplot</code> 預設會畫 bootstrap 信賴區間，"
        "各組區間的重疊不能直接代替差值的推論；要為平均差建立適當的區間或檢定。"
        "區間寬度也受樣本數與資料變異影響，不能只看長條高度。"),
       (False, "只要看兩組的標準差",
...
{hook("這在本站哪一章會用到",
      ...
      '第 3 章的殘差圖更是整章的診斷主力。'
      '<a href="unsupervised_learning.html#biplot">→ 非監督式學習 · 雙標圖</a>　'
      '<a href="unsupervised_learning.html#hclust">→ 非監督式學習 · 樹狀圖</a>')}
"""

# ── P05 把模型畫進圖裡 ─────────────────────────────────────────────────
BODIES["model"] = f"""
  <p>最後一種圖：<strong>把擬合好的模型畫在資料上面</strong>。
...
{card("依類別分別擬合", C(1, 135, 136), src=S(1, 135, 136),
      note="<code>lmplot</code> 加 <code>hue</code> 會<strong>每一組各擬合一條線</strong>。"
...
{qa("觀念釐清", [
    ("<code>regplot</code> 幫我擬合了線，是不是就等於做了迴歸分析？",
...
function w18msDraw() {
  ...
  setStatus('w18msStatus', w18msI === 0
    ...
    : '同樣的兩個數字，截斷之後長條高度看起來差 <b>' + HC.fmt(hB / hA, 1) + ' 倍</b>。資料相同，截斷軸放大了視覺差距。');
}
