...
{card("講義 04 · 用2005 年當測試集", _log_code3, lab_output(CH, 49),
      src=src("45、49、51"),
...
{card("講義 04 · 砍掉沒用的變數再測一次", lab_code(CH, 53), lab_output(CH, 53),
      ...
      note="六個變數的 p 值都很醜，那就只留看起來最有希望的兩個。測試正確率從 48.0% 升到 "
           ...
           "單看整體正確率，無法看出它是否優於多數類基準。這是下面「閾值與混淆矩陣」那一節的引子。")}
...
{quiz("qMul", "QUIZ · 混淆",
      ...
      [(True, "兩個係數在回答不同的問題：後者是「在 balance 相同的前提下」的效果",
       ...
       (False, "這是共線性造成的，把 student 或 balance 移掉一個就好",
        ...
        "（confounding）而不是共線性造成的不穩定——係數的標準誤沒有明顯膨脹，符號翻轉是<strong>真實的、"
        "可解釋的</strong>。移掉變數會改變控制的條件，也會改變係數所回答的問題。")])}
"""
