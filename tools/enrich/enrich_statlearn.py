...
{info("Bayes 錯誤率算不出來，那講它有什麼用", '''<strong>1. 它給出理論上可達到的誤差下限。</strong>
  測試錯誤率已經逼近估計的 Bayes 錯誤率時，可優先考慮蒐集新變數，
...
{info("三個一定要記住的觀念", '''<strong>1. 測試 MSE 永遠不可能低於 Var(ε)。</strong>
  ...
  測試誤差逼近這條下限時，可優先考慮蒐集新變數。<br>
  <strong>2. 訓練誤差單調下降，測試誤差是 U 型。</strong>
  訓練誤差是被最小化過的目標值，它永遠獎勵更多彈性；測試誤差則用未見資料評估表現。
