...
{viz(svg("w02irrSvg", 320),
     [info_card("怎麼玩這個元件",
                '真實的 f（綠色虛線）<strong>固定不動</strong>，只有噪音的 σ 在變。'
                '綠色淡帶是 f ± σ 的範圍。拖滑桿把 σ 拉大，看點雲怎麼變胖——'
                '而 f 一動也沒動。', "ISLP 式 2.3"),
      rows_card("兩種誤差的即時對帳",
                [("σ（噪音的標準差）", "1.0", "w02irrSigma"),
...
{card("講義 02 · 噪音讓相關係數到不了 1", lab_code(CH, 76) + "\n" + lab_code(CH, 78),
...
{quiz("qMSE", "QUIZ · 訓練與測試 MSE",
      ...
      [(True, "選 B。我們要的是在新資料上的表現，A 的訓練與測試差距顯示它在配適噪音",
...
{qa("觀念釐清", [
    ("Q：KNN 的 K = 1 為什麼訓練錯誤率是 0，這代表它很好嗎？",
     ...
     "資料量非常大、噪音非常小的時候，K = 1 是可以贏的——ISLP 第 4 章與第 9 章"
     "會再回到這個取捨。"),
])}
...
{quiz("qKnn", "QUIZ · Bayes 分類器與 KNN",
      ...
      "最合理的解讀是什麼？",
      [(True, "K = 1 太有彈性，決策邊界抓到了訓練資料的噪音；K = 25 平滑掉噪音後更接近真實邊界",
...
{quiz("qEx1", "EXERCISE 1 · ISLP 2.4 第 1 題（b)(d)",
      ...
      [(True, "兩種情況都<strong>比較差</strong>：(b) 資料不夠支撐彈性方法，(d) 彈性方法會去配噪音",
       ...
       (False, "(b) 較差、(d) 較好，因為噪音大的時候更需要彈性去捕捉細節",
        "(d) 的判斷剛好相反。噪音大的時候那些「細節」<strong>就是噪音</strong>，"
        "捕捉它們只會讓測試誤差變大。σ² 大時應該更保守，選偏差稍高但變異低的方法。"),
       (False, "兩種情況都<strong>比較好</strong>，因為彈性方法適用範圍更廣",
        "不對。彈性方法的優勢建立在「有足夠資料」與「訊號比噪音強」這兩個前提上。"
...
function w02flexDraw() {
  ...
  const tag = w02flexDf === 2 ? '太硬：直線配不出這個彎，偏差大'
    : (w02flexDf >= 25 ? '太軟：穿過噪音，訓練 MSE 最小但測試 MSE 最大'
      : '差不多剛好：測試 MSE 接近最低點');
...
function w02knnDraw() {
  ...
  const tag = w02knnK === 1 ? '邊界破碎：每個訓練點都自成一個小島，抓到的是噪音'
