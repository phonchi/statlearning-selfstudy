...
{info("為什麼一定要有 ε 這一項", '''把它拿掉，式子就變成 Y = f(X)，等於宣稱
  ...
  ε 就是這些東西的集合。它的變異數 Var(ε) 決定可達到的誤差下限。這是下一節的主題。''')}
...
{quiz("qIrr", "QUIZ · 兩種誤差",
      ...
      [(True, "可縮減誤差從約 3.2 降到約 0.4，剩下的空間已經很小，可優先考慮蒐集新變數",
        ...
        "已經減少 87%，繼續加彈性的邊際效益很低，而且風險是開始過度配適。"),
...
{card("講義 02 · 用等高線圖看一個指定的 f(x, y)", lab_code(CH, 121), None,
      ...
      note="這一格自己指定了 <code>f = cos(y) / (1 + x²)</code>，"
           ...
           "真正的參數式配適還要指定未知係數，再依資料選出它們；下一章的線性迴歸會完整示範。"
...
{info("三個「選不彈性的」正當理由", '''<strong>1. 要做推論：</strong>
  ...
  提升法給不出這種句子。<br>
  <strong>2. 樣本不夠：</strong>彈性高的方法要餵很多資料才穩，n 小的時候它的變異可能抵銷偏差降低的好處。<br>
...
{info("這張圖的三個一定要看懂的地方", '''<strong>1. 這組巢狀樣條的訓練 MSE 從 3.43 一路掉到 0.48，</strong>
  ...
  <p>這組模型依序擴大且用同一訓練目標配適，所以訓練 MSE 單調不增；
...
BODIES["bayes"] = f"""
  ...
  <p class="core-backfill">條件機率還不熟時，可先回補
  ...
  <p>可以證明（證明超出 ISLP 範圍），母體的期望分類錯誤率會被一個非常簡單的分類器最小化：
...
{qa("觀念釐清", [
    ("Q：本頁 KNN 的 K = 1 為什麼訓練錯誤率是 0，這代表它很好嗎？",
     ...
     "<p>這個 0 本身不能證明過度配適；還要和較不彈性模型的新資料表現比較。"
     "它也不能提供泛化表現的證據，因為使用的是配適時那批資料。"
...
{quiz("qKnn", "QUIZ · Bayes 分類器與 KNN",
      ...
      [(True, "K = 1 太有彈性，決策邊界抓到了訓練資料的雜訊；K = 25 平滑掉雜訊後更接近真實邊界",
       ...
       (False, "K = 1 的訓練錯誤率是 0，所以它一定過度配適；K 愈大一定愈好",
        "兩個「一定」都不成立。K = 1 的訓練錯誤率即使是 0，也要看到較不彈性模型的測試表現更好，才能判定過度配適。K 一直加大還會走到另一個極端："
...
{quiz("qEx3", "EXERCISE 3 · ISLP 2.4 第 3 題（a)(b)",
      ...
      [(True, "偏差²單調下降、變異單調上升、訓練誤差單調下降、測試誤差 U 型、"
        ...
        "訓練誤差<strong>可以</strong>降到水平線之下（甚至到 0），因為這些資料已參與配適；"
...
{table(["", "迴歸（regression）", "分類（classification）"],
       [["Y 是什麼", "數值", "類別"],
        ...
        ["訓練版的問題", "用配適資料自評會偏樂觀", "用配適資料自評會偏樂觀（本例 K = 1 時是 0）"],
