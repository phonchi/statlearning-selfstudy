...
# ── PROLOGUE AI 不只會寫程式 ───────────────────────────────────────────
BODIES["prologue"] = f"""
  <p>你想知道 Auto 汽車資料中的馬力與油耗有什麼關係。AI 可以協助寫繪圖程式、解釋結果。你先確認欄位單位，猜測圖的形狀，再把實際結果和自己的想法比較。</p>
...
{quiz("qPrologue", "PART 00 · 自我檢測",
      ...
      [(False, "把資料貼給 AI，直接採用它寫的結論",
       ...
       (True, "請 AI 產生候選做法，再由你執行、檢查並決定能否採用",
        "對。AI 擴大你能嘗試的範圍，你則用資料與領域知識決定哪些結果有足夠證據支持。"),
       (False, "只把 AI 用在寫語法，其他資料工作完全不用",
        "AI 也能協助搜尋、清理、EDA、文字探勘與報告整理；重點是每一步都要有驗收方法。")])}
"""
...
BODIES["loop"] = f"""
  ...
  AI 可以在每一步提供選項、範例或解釋；這些步驟仍要由你完成。</p>
  ...
  <div class="info-box purple" aria-label="資料分析學習迴圈">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:.75rem;margin:.65rem 0;">
      ...
      <div class="info-card"><div class="ic-title">③ 觀察證據</div>用資料、輸出與錯誤訊息核對 AI 的解釋。</div>
      <div class="info-card"><div class="ic-title">④ 修正理解</div>解釋預測為何不符，改一個條件後重新開始。</div>
    </div>
    <p style="margin:.5rem 0 0;"><strong>↻ 下一輪：</strong>依照這次的結果修正理解，再提出下一個問題。</p>
  </div>

{info("為什麼要先想再問",
      "如果一開始就看完整答案，你很難分辨自己原本懂了什麼。先寫下一句預測，"
      "再比對 AI 的回答和實際輸出，找出需要修正的地方。", "warm")}
...
# ── HABITS 五個學習習慣 ───────────────────────────────────────────────
BODIES["habits"] = f"""
  <p>完成第一張圖後，換一個條件再練：例如只看某些年份的車，先猜圖形會怎麼變，再執行比較。修好之後，試著說明這次改動為什麼有效。</p>
{info("每次練習留下三句話", "① 我原本預期什麼。② 程式與資料實際顯示什麼。③ 我下一次會改哪個條件。這三句比再讀一遍完整答案更容易讓你看出還不懂哪裡。")}
{card("練習讀表：另一份課程示範資料", C(1, 31), O(1, 31), src=S(1, 31), note="這張表取自 Ch01 的小型示範資料，與前面的 Auto 不同。先只指出欄名、資料型態與量級；等讀 Auto 時，也用同樣的方法確認拿到什麼。")}

{quiz("qHabits", "PART 04 · 自我檢測",
      "AI 已經幫你修好一段清理程式，而且能順利執行。怎樣確認你真的學會了？",
      [(False, "把修好的版本收藏起來，下次原樣貼上",
        "收藏答案方便之後查閱。要確認理解，還需要改變條件，自己預測並檢查結果。"),
       (False, "再請 AI 用更簡單的話解釋一次",
        "解釋可以幫忙，但你仍可能只是在熟悉文字。"),
       (True, "改一個欄位型態或特殊值，先預測結果，再自己修改並執行",
        "對。條件改變後仍能預測、操作與解釋，才表示能把所學用在新情況。")])}
...
{quiz("qEx2", "EXERCISE 2 · 文字探勘",
      ...
      [(False, "把四個主題直接當成受訪者的真實觀點",
        "這些主題由分析過程歸納而來，是否符合受訪者的意思仍要回查原文。"),
