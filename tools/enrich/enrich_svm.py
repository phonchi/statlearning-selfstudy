...
{quiz("qHyp", "QUIZ · 超平面",
      ...
      [(True, "超平面本身完全不動，但每個點的 f(x) 變成 −2 倍，正負兩側因此互換",
       ...
       (False, "‖β‖ 不變，所以距離不變，一切照舊",
        "‖β‖ 也變成 2 倍。不過結論反而對了一半：|f(x)| 與 ‖β‖ 都變 2 倍，"
...
{info("一句話的幾何直覺", '''最大邊界超平面就是<strong>能塞進兩類之間那塊最寬「板子」的中線</strong>。
  ...
  <p>兩個限制式各有分工。$\\sum \\beta_j^2 = 1$ 用來固定係數的尺度；縮放係數不改變超平面，
  這項設定可<strong>把 $y_i f(x_i)$ 校準成距離</strong>；
...
{qa("觀念釐清", [
    ...
    ("Q：margin 大在測試資料上真的比較好嗎？",
     ...
     "「這並不令人驚訝」。</p>"
     "<p>所以實務上幾乎沒有人用純粹的最大邊界分類器。常用的是加了正則化的軟邊界版本，"
     "而且那個調整參數要靠交叉驗證選。</p>"),
])}
...
{quiz("qMax", "QUIZ · 最大邊界",
      ...
      [(True, "完全不變。它對應的限制式本來就是鬆的，移動之後更鬆，一樣不影響最佳解",
       ...
       (False, "會稍微改變，因為目標函數是所有點的距離總和",
        "不對。目標函數是 <strong>M</strong>，而 M 定義成所有距離的<strong>最小值</strong>，總和並非這個目標函數。"
...
# ── P02 soft ──────────────────────────────────────────────────────────
BODIES["soft"] = f"""
  <p>上一節結尾的兩個問題要一起解決。第一個是<strong>資料常常無法完全線性分開</strong>
...
BODIES["hinge"] = f"""
  ...
  這一節也提供比較 SVM 與邏輯斯迴歸所需的損失函數觀點。</p>
...
{viz(chart("w10lossChart", "tall", "。此圖的重點：hinge 在 y·f(x) > 1 的平坦區不影響解；等於 1 的折角仍可能是支持向量。邏輯斯損失則永遠大於 0。"),
     [info_card("怎麼看這張圖",
                '<strong>綠線是 hinge</strong> max(0, 1 − y·f)，'
                '<strong>紅線是邏輯斯損失</strong> log(1 + e^(−y·f))。'
                '灰色區帶是 y·f ≥ 1——<strong>綠線在那裡貼在 0 上，紅線逐漸減小並維持大於 0</strong>。'
...
{table(["yᵢf(xᵢ)", "位置", "對解的作用"],
       [["&gt; 1", "在 margin 正確的一側之外", "在損失平坦區，不是支持向量"],
        ["= 1", "剛好在 margin 上", "損失為 0，但位於折角，可能是支持向量"],
        ["&lt; 1", "違反 margin", "損失大於 0，是支持向量"]])}

  <p>支持向量因此來自<strong>margin 上或違反 margin</strong>的觀測值。
  對照之下，邏輯斯迴歸的損失永遠大於 0，每一筆資料都在（微弱地）拉扯答案，
  所以它<strong>沒有</strong>稀疏性、也沒有支持向量的概念。</p>

  <p>按「顯示 0–1 損失」會加上分類錯誤的判定標準：分錯是 1、分對是 0 的
  階梯函數。它不連續也不凸，直接最佳化較困難，所以常改用容易最佳化的
  <strong>凸代理損失</strong>（convex surrogate loss）。hinge 與邏輯斯損失都是代理；
  其中 hinge 是 0–1 損失的逐點上界，本圖未縮放的邏輯斯損失則不一定在階梯之上。</p>
...
{quiz("qHinge", "QUIZ · Hinge loss",
      ...
      [(True, "貢獻是 0；在相同的懲罰設定下，刪掉它仍保留目前的最優解",
       ...
       (False, "貢獻略大於 0，所以刪掉它解會微微改變",
        "這描述<strong>邏輯斯損失</strong>；hinge 在此已進入平坦區。"
        "log(1 + e^(−3)) ≈ 0.049，仍大於 0；hinge 在這裡是<strong>精確的 0</strong>。"
        "兩者的差別就在這裡：邏輯斯迴歸沒有支持向量的概念。")])}
"""

# ── P04 kernel ────────────────────────────────────────────────────────
BODIES["kernel"] = f"""
  <p>軟邊界允許分類錯誤，邊界仍是超平面。遇到<strong>非線性的邊界</strong>時，還需要擴充特徵。
  ...
  問題是維度增加得很快：講義第 21 頁做到三次多項式，兩個變數就變成九維；
...
{card("lab 09 · 兩條路的答案一模一樣", lab_code(CH, 53), lab_output(CH, 53),
      ...
      note="<code>SVC(kernel=my_kernel_1)</code>（只用核、留在二維）與 "
           ...
           "<strong>這個比較呈現了核技巧的等價性</strong>：兩條路數學上等價，"
  ...
  <p><strong>徑向基核</strong>（radial basis kernel，RBF；式 9.24）依兩點間的平方距離定義：</p>
...
{info("兩個關鍵差別", '''<strong>① 分類器的個數：</strong>OVO 是 K(K−1)/2，OVA 是 K。
  ...
  「其餘」——樣本不平衡，而且那一團的形狀可能無法用一個超平面分開。<br>
...
{table(["", "支持向量分類器 / SVM", "邏輯斯迴歸", "LDA"],
       [["損失函數", "hinge：max(0, 1 − y·f)", "log(1 + e^(−y·f))", "（此欄列出概似）"],
...
{card("lab 09 · RBFSampler + SGDClassifier",
      ...
      note="<code>RBFSampler</code> 用隨機傅立葉特徵<strong>近似</strong> RBF 核："
           ...
           "訓練準確率 <strong>0.84</strong> 低於 <code>SVC(kernel='rbf')</code>，"
           "但它是 O(n) 的，資料上百萬筆時 <code>SVC</code> 計算成本可能過高。"
...
{quiz("qEx2", "EXERCISE 2 · ISLP 9.7 第 2 題（c）（d）",
      ...
      [(True, "藍、紅、藍、藍；(d) 對 X₁、X₂ 為非線性；對 X₁、X₁²、X₂、X₂² 則為線性",
...
{quiz("qEx4", "EXERCISE 4 · ISLP 9.7 第 6 題",
      ...
      [(True, "小 C（會錯分幾筆訓練資料）在測試資料上可能贏過大 C，因為大 C 的邊界只由極少數點決定、變異大",
       ...
       (False, "大 C 一定更好，因為它的訓練誤差是 0，而訓練誤差 0 表示模型完全學會了資料",
        "訓練誤差 0 仍需配合泛化評估解讀。第 2 章與第 5 章反覆講過這件事。"
...
{info("重點回顧", '''<strong>1. 解只依賴支持向量。</strong>
...
function w10hyperDraw() {
  ...
  if (nrm < 1e-9) {
    setStatus('w10hyperStatus', 'β₁ 與 β₂ 同時是 0：方程式退化成 β₀ = 0，'
      + '<b>這組係數沒有定義出超平面</b>。把任一個滑桿推離 0 就回來了。');
  } else {
...
function w10lossDraw() {
  ...
  setStatus('w10lossStatus', 'y·f 從 1 往右，hinge 精確等於 0（灰色區帶），'
    ...
    + '嚴格大於 1 的點在平坦區；等於 1 的折角仍可能是支持向量。'
    + (w10lossShow01 ? ' 灰色階梯是 0–1 損失。兩條曲線都是凸代理；只有 hinge 在本圖中是逐點上界。' : ''));
}
