...
{qa("常見疑問", [
    ("那我可以只用 pandas，不學 NumPy 嗎？",
     "學會 NumPy 的陣列概念，也有助於理解 pandas。<code>DataFrame</code> 的每一欄底下就是一個 NumPy 陣列，"
     "<code>df['mpg'].mean()</code> 走的是 NumPy 的路。"
     "pandas 管理欄名與索引，數值運算則會用到 NumPy。"),
    ("陣列比串列快在哪裡？",
...
# ── P01 建立陣列與 shape ────────────────────────────────────────────────
BODIES["create"] = f"""
  <p>讀取陣列時，先看三個屬性：<code>ndim</code>（幾維）、<code>dtype</code>（裝什麼型別）、
  <code>shape</code>（每一維多長）。看不懂錯誤訊息的時候，先印出這三個屬性，確認維度、型別與形狀。</p>
...
{quiz("qView", "PART 02 · 自我檢測",
      ...
      [(False, "還是 1，y 是新的陣列",
       ...
       (True, "99，因為 y 跟 x 共用同一塊資料",
        ...
        "資料前處理時要確認是否共用記憶體，修改暫存變數也可能影響原始資料。"),
       (False, "會報錯，view 不能改",
...
# ── P03 索引、切片與子矩陣 ──────────────────────────────────────────────
BODIES["index"] = f"""
  <p>同時指定列與欄時，要看清楚索引如何配對。lab 裡有一格叫
...
# ── P04 布林索引 ────────────────────────────────────────────────────────
BODIES["bool"] = f"""
  <p>統計程式常依<strong>條件</strong>選取資料：
...
{viz(svg("w16maskSvg", 320),
     [info_card("拖門檻",
      ...
      info_card("比較迴圈與布林遮罩",
                "布林遮罩可以直接表達整組資料的篩選規則。"
                "<strong>集中寫出篩選條件，也方便看出選取規則</strong>，"
                "半年後回來看還讀得懂。")],
...
{quiz("qMask", "PART 04 · 自我檢測",
      ...
      [(True, "392，每一列一個 True 或 False",
        ...
        "符合條件的筆數則用 <code>mask.sum()</code> 計算。"),
       (False, "符合條件的筆數，例如 89",
...
# ── P06 彙總與 axis ─────────────────────────────────────────────────────
BODIES["agg"] = f"""
  <p>理解 <code>axis</code> 時，可以先看彙總沿著哪一維進行：
...
{hook("這在本站哪一章會用到",
      ...
      '<a href="statistical_learning.html#mse">→ 統計學習 · 評估擬合品質</a>')}
"""
...
{quiz("qSeed", "PART 07 · 自我檢測",
      ...
      [(False, "自助法本身有 bug",
        "先核對資料與重現性設定，確認兩次使用的樣本和種子，再檢查演算法實作。"),
...
{table(["你想做的事", "寫法", "回傳的是檢視還是複本"],
       [["取連續的列", "<code>A[1:3]</code>", "檢視"],
        ...
        ["換形狀", "<code>A.reshape((2, 3))</code>", "可行時共用資料，否則複製"],
        ["轉置", "<code>A.T</code>", "檢視"],
