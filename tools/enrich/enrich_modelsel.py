...
{viz(chart("w06hdChart", "tall", "。此圖的重點：n = 20、p 從 1 增到 19（加截距共 20 個參數）時，訓練 R² 一路衝到 1，而測試 MSE 從 1.07 爆到 46.3。"),
     [info_card("上圖在做什麼",
      ...
      info_card("維度的詛咒",
                '這裡所有變數都無法提供母體的預測訊號，但最小平方仍能改善訓練配適。'
...
{info("在高維度千萬不要相信這三個數字", '''<strong>1. 訓練資料上的 R²。</strong>
  模型有足夠自由度時它可能接近 1，但不能據此判斷泛化。<br>
  <strong>2. 訓練資料上的 RSS 或 MSE。</strong>同理，可能接近 0，卻沒有告訴你測試誤差。<br>
...
{quiz("qHd", "QUIZ · 高維度",
      ...
      [(True, "訓練 R² = 1，但不能因此判斷新資料的預測表現",
        "對。列滿秩讓模型能插值這 50 筆資料，但係數不唯一；不同選解規則可能給出不同的新資料預測。收縮、降維與獨立評估才是在處理如何選模型的問題。"),
       (False, "訓練 R² 接近 0，因為變數比樣本多",
        "在題目給的列滿秩條件下，最小平方可以把訓練 RSS 壓成 0。變數多不會阻止插值，反而讓訓練滿分失去判斷泛化的能力。"),
       (False, "無法計算任何配適值，因為係數解不唯一",
        "係數不唯一不代表配適值算不出來。可以用偽逆選出一組解；但這個選解規則與它的測試表現，不能只靠訓練 R² 證明。")])}
