...
<p>當中心化設計矩陣滿足 $X^TX=I$，令 $z=X^Ty$，Ridge 解為 $z_j/(1+\lambda)$，Lasso 解為 $\operatorname{sign}(z_j)(|z_j|-\lambda/2)_+$。Ridge 比例收縮、Lasso 軟閾值的差異可直接從這兩式看見；若目標使用 $\mathrm{RSS}/2$，Lasso 的閾值相應改為 $\lambda$。</p>
...
"""
BODIES['criteria'] += r"""
<h3>分類模型的模型選擇</h3><p>子集搜尋也能套用邏輯斯迴歸等模型，但比較的配適量要改為相應的概似。離差（deviance）定義為 $D=2(\ell_{\mathrm{sat}}-\ell_{\mathrm{fit}})$；它比較配適模型與飽和模型，不能直接把分類誤差率代入 RSS 公式。AIC 為 $-2\ell_{\mathrm{fit}}+2k$，BIC 為 $-2\ell_{\mathrm{fit}}+k\log n$，$k$ 計入實際估計的參數。選定模型後的係數不確定性還受搜尋影響，不能忽略選模就直接沿用事先固定模型的推論。</p>
"""
