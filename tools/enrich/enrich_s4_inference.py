...
BODIES["intervals"] = rf"""
  ...
  先看條件最單純的情況：$X_1,\ldots,X_n$ 獨立來自常態母體，母體標準差 $\sigma$ 已知。</p>
...
{info("95% 說的是長期涵蓋率", "若一再用同一套抽樣與建立區間的程序，長期約 95% 的區間會涵蓋固定的真實平均數。區間算出後，端點與母體參數都已固定；不能再說這一條區間有 95% 機率包含它。", "warm")}
...
{quiz("qCI", "QUIZ · 信賴區間",
      ...
      [(False, "真實平均有 95% 機率落在 48.08 到 55.92",
        "區間算出後，參數與端點都固定。95% 描述建立區間的程序反覆使用時的長期涵蓋率。"),
       (True, "同樣程序反覆抽樣並建立區間，長期約 95% 的區間會涵蓋真實平均",
...
{source_note("現行網頁的 The Bootstrap 示範從經驗分佈有放回重抽並估樣本平均的標準誤。", ST_WEB, "Seeing Theory 網頁 · The Bootstrap")}
"""
