...
{_quiz('Wage', '教育程度較高的組，薪資中位數也較高。這張圖支持哪個說法？', '樣本中不同教育組的薪資分布不同，但不能直接推論教育造成差距', '對。箱形圖比較組間與組內差異，並未控制其他因素。', '同一教育組的人薪資都相同', '不對。箱子、鬚與外面的點正是在呈現組內差異。', '多讀一個教育階段，每個人的薪資都會增加固定金額', '不對。這些是不同人的比較，且教育組之間的差距也不固定。')}
...
<p>這份資料另有癌症型別標籤，但眼前的任務是只根據基因表現探索結構，因此投影時不把型別當作y。</p>
{nci}
