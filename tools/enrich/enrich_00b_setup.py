...
{table(["情境", "建議"],
       [["第一次上手、只想跑 lab", "Colab"],
        ...
        ["期中考前", "在電腦教室的電腦完整跑一次練習，熟悉開啟檔案、路徑與 kernel"]])}
...
{qa("還是跑不動的話", [
    ("問人之前先做這三件事",
     "① <strong>重啟並全部重跑</strong>；② 複製<strong>完整 traceback</strong>，先看最後一行，並保留最靠近自己程式的出錯位置；"
     "③ 確認是哪一格出錯、那一格用到哪些變數。"
...
{quiz("qFix", "PART 05 · 自我檢測",
      "相同 notebook 在不同執行環境得到不同結果，應先如何檢查？",
      [(True, "先從乾淨 kernel 全部重跑，再比資料、前處理、版本與 seed",
       ...
       (False, "一定是其中一邊沒有固定 seed",
        ...
        "kernel 與亂序執行都可能造成差異，不能只由結果不同反推原因。"),
