...
const stems=requested.length?requested:fs.readdirSync(ROOT).filter(n=>n.endsWith('.html')).map(n=>n.slice(0,-5)).sort();
fs.mkdirSync(SHOTS,{recursive:true});
(async()=>{
 const b=await pup.launch({executablePath:chrome,headless:true,args:['--no-sandbox','--disable-dev-shm-usage']});
 const prior=path.join(OUT,'final-views.json');
 const records=requested.length&&fs.existsSync(prior)?JSON.parse(fs.readFileSync(prior)).pages.filter(p=>!requested.includes(p.stem)):[];let cursor=0;
 try{
