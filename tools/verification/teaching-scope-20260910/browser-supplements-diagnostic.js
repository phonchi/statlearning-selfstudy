...
async function checkOne(browser, stem) {
  ...
  for (const b of btns) {
    ...
    catch (e) { note(stem, '控制鈕無法操作：' + await b.evaluate(el => el.textContent.trim()) + ' / ' + e.message + ' / ' + JSON.stringify(await b.evaluate(el => ({ancestors: [...(function*(n){while(n){if(n.tagName==="DETAILS")yield {id:n.id,open:n.open};n=n.parentElement;}})(el)],rect:el.getBoundingClientRect().toJSON()})))); }
  }
  // select 推到每個選項
