...
try{for(const [stem,id] of [['support_vector_machines','w10-proof-hard-dual'],['support_vector_machines','w10-proof-soft-dual']]){
...
await p.$eval('#'+id,e=>{e.open=true;});await new Promise(r=>setTimeout(r,1100));await (await p.$('#'+id)).screenshot({path:path.join(__dirname,'final-reading',id+'.png')});
