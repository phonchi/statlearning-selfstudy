...
HC.base = (type, data, opts = {}) => {
  ...
  return { type, data, options: deep(defaults, opts) };
};
...
/* 自製 inline plugin：不需要下載 chartjs-plugin-annotation */
HC.hline = (y, label, color) => ({
  id: 'hline_' + y + '_' + (label || ''),
  afterDatasetsDraw(chart) {
    ...
    ctx.save();
    ctx.strokeStyle = color || HC.tok.muted;
    ctx.lineWidth = 1.4; ctx.setLineDash([5, 4]);
    ctx.beginPath(); ctx.moveTo(ca.left, py); ctx.lineTo(ca.right, py); ctx.stroke();
    if (label) {
      ctx.setLineDash([]); ctx.font = '600 10px ' + HC.MONO;
      ctx.fillStyle = color || HC.tok.muted; ctx.textAlign = 'right'; ctx.textBaseline = 'bottom';
...
HC.vline = (x, label, color) => ({
  ...
  afterDatasetsDraw(chart) {
    ...
    ctx.save();
    ctx.strokeStyle = color || HC.tok.accent;
    ctx.lineWidth = 1.6; ctx.setLineDash([5, 4]);
    ctx.beginPath(); ctx.moveTo(px, ca.top); ctx.lineTo(px, ca.bottom); ctx.stroke();
    if (label) {
      ctx.setLineDash([]); ctx.font = '600 10px ' + HC.MONO;
      ctx.fillStyle = color || HC.tok.accent; ctx.textAlign = 'left'; ctx.textBaseline = 'top';
      ctx.fillText(label, px + 4, ca.top + 2);
    }
    ctx.restore();
  },
});

HC.vband = (x0, x1, fill, label) => ({
  id: 'vband_' + x0 + '_' + x1,
  beforeDatasetsDraw(chart) {
    const { ctx, chartArea: ca, scales } = chart;
    if (!scales.x) return;
    const a = scales.x.getPixelForValue(x0), b = scales.x.getPixelForValue(x1);
    ctx.save();
    ctx.fillStyle = fill || 'rgba(44,62,122,.10)';
    ...
    if (label) {
      ...
      ctx.fillText(label, (a + b) / 2, ca.top + 2);
    }
    ctx.restore();
  },
});
