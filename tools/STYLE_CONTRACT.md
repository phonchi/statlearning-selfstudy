...
(b) 忠實擬合需要無法用 50 行 JS 重寫的套件（glmnet 路徑、`SVC` 軟邊界、`scipy` linkage、
...
- 移除沒有教學資訊的宣示、誇張比喻與編修歷史。數字、公式、程式與必要的邏輯否定要保留，語氣調整不能改變結論與限制。
- **講義與課本的英文敘述一律用中文轉述，不保留英文原句**（見下方「英文的去留」）。轉述要保住原句的限定詞——`typically`→「通常」、`can be`→「可以」、`need no longer be`→「不再算是」——不可以翻成絕對句。
...
- **禁止中國用語**：軟件→軟體、函數（可）／函式（可）但同頁一致、缺省→預設、
  ...
  默認→預設、複雜度（可）、fitting 統一用「擬合」，包括「過度擬合」「欠擬合」「擬合值」；不混用其他譯名。
- Emoji 只當標籤前綴：📌 📑 📓 📖 📗 🏠 🔗 ▶ 🔀。
- 每個 quiz 的錯選項都要解釋原因，直接指出需要修正的推論或使用條件。保留題目的真偽與答案位置。

### 英文的去留

站是寫給中文讀者的。**達到一個句子或子句長度的英文敘述一律翻成中文**，
包括講義原句、課本原句、習題題幹片段，以及夾在中文句子裡的英文子句
（「講義那句 the expectation averages over …」這種寫法）。
引言框的標籤跟著改，不要留「原句」兩個字卻放中文。

**維持英文原文**：

| 類別 | 例子 |
|---|---|
| 括號裡的術語對照（首次出現寫「中文（English）」） | 迴歸函數（regression function） |
| 書名、作者、正式名稱 | `sources.py` 的 `BOOKS`、`pages.py` 的 `title_en` |
| 程式碼、lab 儲存格、`.expected-out` 的逐字輸出、直譯器錯誤訊息 | `ValueError: The truth value of a Series is ambiguous` |
| 套件／函式／參數／識別字 | `scikit-learn`、`ddof`、`FRAMES_w02bv` |
| 數學符號與 LaTeX | `\arg\max`、`E[Y \mid X = x]` |
| 詞彙卡的英文**術語**欄 | 維度詛咒（Curse of Dimensionality） |

`tools/check_english_prose.py` 會掃出讀者可見文字裡連續五個以上的英文單字。
它只報告不擋，allowlist 直接讀 `BOOKS` 與 `title_en`，不要另外手抄一份。

### 30 詞術語表（同一頁內用詞要一致）

迴歸 · 分類 · 變異數 · 標準差 · 標準誤 · 常態分佈 · 期望值 · 偏差 · 變異 ·
過度擬合 · 擬合 · 殘差 · 槓桿值 · 共線性 · 交叉驗證 · 折 · 重抽樣 · 自助法 ·
正則化 · 收縮 · 稀疏 · 調整參數 · 超參數 · 決策邊界 · 混淆矩陣 · 敏感度 ·
...
| Q&A | 3–4 則（第 3 章 6 則） |
| `.deck-extra` | 8–12 張 |
| 詞彙卡 | 僅保留本頁涉及的統計／機器學習常用專有名詞；不設最低張數 |
...
- 課程資料圖須依 lab 的資料範圍、前處理、估計方法、參數與圖層呈現；不可自行抽樣、改分箱平均、合併類別或省略密度／信賴區間。額外的概念互動需明確標示用途。
...
## 13. 台灣教學用語（2026-09-08）

- fitting 統一使用「擬合」；動作寫「擬合模型」「重新擬合」，不省寫成「配一下」「重配」等不清楚的說法。
- likelihood 用「概似」（最大概似、對數概似、條件概似），sensitivity 用「敏感度」，sample size 用「樣本數」。
- threshold 用「門檻值」；workflow 用「操作流程／分析流程」；baked data 在讀者文字用「預先計算的資料」。
- 保留學科上的語意差別：「機率質量」不能改成「機率品質」，「水平線」不能改成「水準線」；regularization 與 normalization 不得因換詞混為一談。
- 套用 speak-human-tw 時，逐段檢查標題、正文、圖說、動態旁白、回饋與詞彙卡，並保留完整的原句／改後句／原因記錄。
- 原始 lab 程式、保存輸出、直譯器訊息及正式名稱保留原文；**講義／課本的英文敘述改用中文轉述**（§6「英文的去留」）。數字、公式、答案與運算邏輯不得因潤稿改變。
- 用 tools/check_taiwan_wording.py 阻擋已確認不採用的讀者用詞，再執行保真與生成檢查。


## 14. 導論的知識順序

- 導論以讀圖為主，不預設學生已學過類別變數編碼、對比、截距、GLM 或 bootstrap。模型例子只說明圖在比較什麼、縱軸的意思與必要限制，計算細節連到對應章節。
- 使用「信賴區間」，曲線圖可稱「逐點信賴區間」；保留它與整條曲線同時涵蓋、個別觀測預測區間的差別。
- 用「相對於平均水準的差異」解讀第四章的月份／時段係數，不將計算約束自創為導論術語。


## 15. 詞彙卡資格

- 詞彙卡只收統計或機器學習常用的專有名詞；一般描述、閱讀提示、論文標題、文章摘要、環境操作與純程式語法不列為詞彙。
- 不改名包裝非術語卡，不以張數湊內容；沒有合適名詞時使用空陣列，產生器移除詞彙卡區塊及導覽。
- 保留卡的定義也需正確，不能只憑標題通過。刪除與修訂應保留逐卡理由。


## 11. 完整自學與證明收合（2026-09-10）

- 每個講義及直接相關教材主題需有完整定義、公式與條件、可跟做的演算法或算例；出現在詞彙卡、速查表或來源徽章不等於正文已涵蓋。來源及修訂對照保存在 `tools/verification/coverage-20260910/`。
- 所有一般性證明／推導使用 `lib.proof(pid, title, body)`，輸出不帶 `open` 的 `details.qa-item.proof`，預設收合。結果、條件、直覺、演算法步驟與例題代入計算留在正文；已有完整證明應遷移而非重寫成摘要。
- proof id 沿用頁面 wNN 前綴；摘要直接寫出待證結論。沿用現有 details 的 MathJax 重排，不另加動畫或狀態系統。
- 不因 SIZE 建議警告刪掉必要教材。保留原有版型與圖表，長公式在手機上於公式區內水平捲動。
- 外部原文受阻須記錄實際錯誤與權威替代；不得把未讀原文寫成已核讀。原來源有錯時以原始教材及獨立推導核正。
- 驗收包括預設收合、Enter/Space、展開後 MathJax、桌面/手機截圖與語意內容核對；自動結構檢查不代替教材完整性判斷。


## 12. 全站閱讀層次（2026-09-10，後續使用者修訂）

- 正文採「直覺＋核心公式＋短例」，保留核心理解需要的條件、主要互動與概念自測。完整計算、長表、成套Lab及進階方法使用 `lib.detail(pid, title, body)` 預設收合；不用字數或表格列數機械判斷。
- 一個主題為一個完整單元：相關說明、公式、例子、程式、輸出、證明與計算題一起安排。既有證明仍收合，最多形成「閱讀細節→證明」兩層。正文不能引用尚未展開的上式／下圖，也不能考未提供前提的計算題。
- 已指定刪除的導論手算、中位數效率與表格合併段不恢復；導論只保留摘要統計量直覺。Ch2高維幾何細節留在收合中。
- `HC.onDetail(id, {open, close})` 管理必要的互動回呼；關閉要停播放並同步按鈕，保留當前狀態，不在再次打開時自動播放。深連結先展開祖先，再定位。
- 公式中的小於號使用 `\lt` 或HTML字元參照，避免HTML解析把數學當成標籤。不得更動Lab原始程式與輸出以消除顯示檢查；需保護code／pre範圍。
- Ch1–6的內容組裝使用 beautifulsoup4（bs4），與既有Lab渲染驗證相同；執行該組enrich前環境需能import bs4。
- `check_reading_flow.py`檢查指定刪除、細節預設狀態、層級與錨點；browser_check先測預設閱讀，再展開細節操作，不能跳過隱藏控制項。實際閱讀判斷另記錄於reading-flow驗證目錄。


## 13. 教學範圍與來源（2026-09-10，最新使用者修訂）

- 以講義實際教學為準。原公式、必要推導與直接教方法的原課程Lab保留；只列名的方法保留於收合，不循外鏈另開其他教材。統計附錄依Seeing Theory，深度學習補充依ISLP Ch10與官方Lab。
- 不新增自訂程式、數值算例、資料或實驗。原Lab中的必要片段可以保留，原教材數字的必要解說也保留；不能只因段落有數字就刪除，需逐項核對來源。
- 非原例的數值算例及其計算題刪除，不補替代題湊數。必要概念互動圖允許示意資料，但不能當作實測排名或新增模型比較；圖的單位、軸與必要條件仍要清楚。
- 網站製作／重現／版本／generator／對值報告不放學生頁，包括收合區。內部provenance與驗證紀錄另保存。安裝頁的真實版本需求、原Lab正在教的亂數種子等仍按教學用途保留。
- 不要求每頁有程式卡、每節有固定題數或可見ver-note。空練習區以Page.show_exercises=False移除。現存程式與輸出仍由原來源核對，不放寬來源驗證。
- 使用HTML解析器整理程式卡時必須保留pre/textarea/span/div內的空白；不能以正規化後相同取代原Lab縮排／文字核對。
- 遇到確實無法判斷的來源／必要性問題先問使用者，不能以概念相關、字數或關鍵字代替判斷。已明確指定刪除的內容不得改名或折疊放回。
