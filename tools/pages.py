...
PAGES = [
    ...
    Page(
        ...
        formula="平時可用 Colab｜期中使用電腦教室電腦｜本機與教室使用相同版本｜確認 kernel 與資料路徑｜考前完整重跑練習",
        deck="", deck_pages=0, lab="",
    ...
    Page(
        ...
        secs=[
            ...
            Sec("toolchain", "繼續學習", "開始練習 Lab，繼續學習後續章節", "講義 01 · p.36、38–39"),
        ],
    ),
    Page(
        ...
        formula="Y = f(X) + ε｜f(x) = E(Y|X = x)｜可縮減 vs 不可縮減｜鄰域平均與維度詛咒｜偏差² + 變異 + Var(ε)｜Bayes 分類器｜KNN",
        ...
        secs=[
            ...
            Sec("irreducible", "兩種誤差", "可縮減與<strong>不可縮減</strong>誤差的來源",
                "ISLP §2.1.1|講義 02 · p.11"),
            Sec("regfunc", "理想的 f 與最近鄰平均",
                "資料點夠就直接取條件期望；不夠才退到鄰域平均",
                "ISLP §2.1|講義 02 · p.10–12"),
            Sec("curse", "維度詛咒", "為什麼高維空間裡「最近的鄰居」一點都不近",
                "ESL §2.5|講義 02 · p.13–15"),
            ...
            Sec("mse", "訓練與測試 MSE", "用訓練 MSE 與測試 MSE 比較模型彈性", "ISLP §2.2.1|講義 02 · p.24–27"),
    ...
    Page(
        ...
        secs=[
            ...
            Sec("inference", "係數的不確定性", "SE、信賴區間與 t 檢定：這個斜率可信嗎",
                "ISLP §3.1.2、3.2.2|講義 03 · p.7–11、23–24"),
    ...
    Page(
        ...
        secs=[
            ...
            Sec("qda", "QDA 與 Naive Bayes", "共變異數要不要共用？獨立假設又幫了什麼",
                "ISLP §4.4.3–4.4.4|講義 04 · p.33–37"),
            Sec("threshold", "門檻值、混淆矩陣與 ROC", "調整 0.5 門檻值，觀察各類錯誤如何改變",
                "ISLP §4.4.2|講義 04 · p.29–32"),
    ...
    Page(
        ...
        plain="線性模型選擇與正則化",
        page_css="#reference .cmp-table{min-width:640px;}",
        ...
        secs=[
            ...
            Sec("subset", "子集選擇", "如何比較 2^p 個候選模型：最佳子集與逐步選擇",
                "ISLP §6.1|講義 06 · p.6–16"),
            Sec("criteria", "選模型的準則", "Cp、AIC、BIC、調整後 R²：懲罰項在做什麼",
                "ISLP §6.1.3|講義 06 · p.17–24"),
            ...
            Sec("lasso", "Lasso 與稀疏性", "L1 為什麼會把係數壓成剛好 0", "ISLP §6.2.2|講義 06 · p.32–41"),
    ...
    Page(
        ...
        secs=[
            ...
            Sec("poly", "多項式迴歸", "高次多項式的擬合與邊界變動", "ISLP §7.1|講義 07 · p.5–7"),
            ...
            Sec("natural", "自然樣條", "自然樣條：以線性約束處理兩端的擬合",
                "ISLP §7.4.4–7.4.5|講義 07 · p.18–23"),
    ...
    Page(
        ...
        secs=[
            ...
            Sec("kernel", "核技巧", "不用真的升維也能畫出彎的邊界", "ISLP §9.3|講義 09 · p.19–28"),
    ...
    Page(
        ...
        secs=[
            ...
            Sec("rnn", "文件分類與 RNN", "文件分類中的詞袋表示與順序資訊",
                "ISLP §10.4|ISLP §10.5"),
            Sec("fitting", "怎麼擬合", "反向傳播、SGD、dropout：神經網路的訓練流程",
    ...
    Page(
        ...
        hero_svg=_svg_pipe(),
        group="appendix",
        kind="prep", data_key="prep_p6_modeling_api", src_labs=(3, 5, 6),
