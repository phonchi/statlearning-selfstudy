...
def card(p: P.Page):
    return (f'      <a class="ch-card" href="{p.file}">\n'
            f'        <span class="ch-num">{p.n:02d} · {p.slug}</span>\n'
            f'        <h3>{p.plain}</h3>\n'
...
<meta name="description" content="統計學習與資料探勘（ISLP）互動自學網站：十章互動教材，每節都能動手操作、預測、驗證。NSYSU MATH524 課程配套。">
...
<div class="hero" id="top">
  ...
  <div class="hero-content">
    ...
    <div class="big-formula">迴歸｜分類｜重抽樣｜模型選擇｜非監督式｜樣條與 GAM｜樹與集成｜SVM</div>
    <div class="scroll-hint">選一章開始<span>↓</span></div>
  </div>
</div>
...
<div class="container">
  <section>
    <div class="loop-box">
      ...
      每頁的 §徽章都標了 ISLP 節號與講義頁碼，方便左右對照；程式碼與「預期輸出」都逐字取自課程 lab 的實跑結果。</p>
...
def build_readme():
    ...
    for p in P.PAGES:
        ...
        rows.append(f"| {p.n:02d} | [{p.plain}]({p.file}) | {對應} | 講義 {p.deck_no} | "
